//! Per-Layer (n-gram) Embedding kernels: hashed ids, quantized table gather,
...
/// Device-resident hashing constants.
pub struct PleHashTables {
    /// U32 `[2 * ngram]`: each 64-bit multiplier as two little-endian words.
    pub multipliers: Tensor,
    /// U32 `[heads]`.
    pub sizes: Tensor,
    /// U32 `[heads]`.
    pub offsets: Tensor,
...
impl PleHashTables {
    pub fn new(
        ...
        let words: Vec<u32> = multipliers
            .iter()
            .flat_map(|&m| [(m & 0xffff_ffff) as u32, (m >> 32) as u32])
            .collect();
        let as_u32 = |v: &[u64]| v.iter().map(|&x| x as u32).collect::<Vec<u32>>();
        Ok(Self {
            multipliers: Tensor::from_bytes(
                ctx,
                bytemuck::cast_slice(&words),
                &[words.len()],
                DType::U32,
            )?,
            sizes: Tensor::from_bytes(
                ctx,
                bytemuck::cast_slice(&as_u32(sizes)),
                &[sizes.len()],
                DType::U32,
            )?,
            offsets: Tensor::from_bytes(
                ctx,
                bytemuck::cast_slice(&as_u32(offsets)),
                &[offsets.len()],
                DType::U32,
            )?,
            heads: sizes.len(),
...
/// `ids[m, j]` for `tokens` (U32 `[M]`) given the two-token history `hist`
/// (U32 `[2]`, oldest first).
pub fn ple_hash_ids(
    ctx: &MetalContext,
    pass: &ComputePass<'_>,
    tables: &PleHashTables,
    tokens: &Tensor,
    hist: &Tensor,
    ids: &Tensor,
) -> Result<()> {
    let m = tokens.numel();
    ensure!(m > 0 && tokens.dtype() == DType::U32, "tokens must be U32 [M]");
    ensure!(hist.numel() == 2 && hist.dtype() == DType::U32, "hist must be U32 [2]");
    ensure!(
        ids.numel() == m * tables.heads && ids.dtype() == DType::U32,
...
/// Advances the two-token history past `tokens`; `hist_in` and `hist_out`
/// may alias (the kernel reads before it writes).
pub fn ple_hist_update(
    ctx: &MetalContext,
    pass: &ComputePass<'_>,
    tokens: &Tensor,
    hist_in: &Tensor,
    ...
    for (name, t) in [("hist_in", hist_in), ("hist_out", hist_out)] {
        ensure!(t.numel() == 2 && t.dtype() == DType::U32, "{name} must be U32 [2]");
    }
    let pipeline = ctx.pipeline("ple_hist_update", SOURCE, MslVersion::V3_1)?;
    pass.dispatch_at(
        &pipeline,
        &[tokens.binding(), hist_in.binding(), hist_out.binding()],
        &[&u32_bytes(m)],
